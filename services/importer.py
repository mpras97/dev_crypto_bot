...
class Importer:
    ...
    def process(self):
        for price in self.exchange.historical_symbol_ticker_candle(
            self.period_start, self.period_end, self.interval):
            logging.info(price.create({"dataset": "/api/datasets/"+self.dataset.uuid}))
